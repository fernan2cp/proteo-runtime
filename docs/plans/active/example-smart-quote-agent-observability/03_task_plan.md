...
- Implement `examples/smart_quote_agent/telemetry_db.py`:
  ...
  - Typed helper functions for inserting records into each table (`record_runtime_event`, `record_langsmith_create_run`, `record_langsmith_update_run`, `record_otel_span`, `record_otel_metrics`).
  - Helper functions for querying invocation summaries and individual event streams.
