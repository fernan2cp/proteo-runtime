...
All tasks are currently in `pending` status. Implementation begins once this SDD package is approved.

---

## Phase 1: Foundational Core Contracts & Profile Declarations

- [ ] **CAL-TASK-0001**: Declare `controlled_turn` profile specification in `src/proteo_runtime/core/profiles.py`.
...
- [ ] **CAL-TASK-0002**: Update `controlled_agent` profile specification to task-scoped runtime context in `src/proteo_runtime/core/profiles.py`.
  - Update `controlled_agent` entry in `DEFAULT_PROFILE_SPECS` to use `ContextPolicy.RUNTIME`.
  - *Maps to*: CAL-REQ-002, CAL-REQ-003.

- [ ] **CAL-TASK-0003**: Update `RuntimeConfigV1` validation model in `src/proteo_runtime/config/models.py`.
  - Allow profile configurations combining `LifecycleMode.EPHEMERAL` with `ContextPolicy.RUNTIME`.
  - *Maps to*: CAL-REQ-004.

- [ ] **CAL-TASK-0004**: Add default Codex mappings for `controlled_turn` in `src/proteo_runtime/config/defaults/codex_v1.json`.
  - Declare model and reasoning effort mappings for levels `low`, `medium`, `high`, and `ultra`.
  - *Maps to*: CAL-REQ-005, CAL-REQ-006.

- [ ] **CAL-TASK-0005**: Implement `RuntimeTask[T]` Protocol contract in `src/proteo_runtime/core/task.py`.
...
- [ ] **CAL-TASK-0006**: Add `task(...)` and `get_task(...)` factory methods to `Runtime` protocol in `src/proteo_runtime/core/runtime.py`.
...
- [ ] **CAL-TASK-0007**: Add `ephemeral_tasks: bool = False` to `RuntimeCapabilities` and update supporting provider runtimes.
...
- [ ] **CAL-TASK-0008**: Add runtime events, result attributes, and `ToolRequest.task_id` correlation.
...
- [ ] **CAL-TASK-0009**: Enforce factory lifecycle boundaries and mandatory tool bindings across `model()`, `task()`, and `session()`.
...
- [ ] **CAL-TASK-0010**: Implement active in-memory task registry `_tasks` and non-resumability guard in `CodexRuntime` and `FakeRuntime`.
...
- [ ] **CAL-TASK-0011**: Implement `task(..., instructions=...)` in `FakeRuntime`.
...
- [ ] **CAL-TASK-0012**: Implement `task(..., instructions=...)` in `CodexRuntime` with compatibility bridge wire translation.
...
- [ ] **CAL-TASK-0013**: Implement bounded `_StatefulContext` helper and single-active-turn locking.
  - Implement single-turn locking using `asyncio.Lock`, raising `SessionBusyError("Task already has an active turn")` on conflict.
  - *Maps to*: CAL-REQ-010.

- [ ] **CAL-TASK-0013b**: Implement hardened context replay protection in `_StatefulContext` / `RuntimeTask`.
...
- [ ] **CAL-TASK-0014**: Implement task workspace directory lifecycle.
  - Create isolated temporary directory on task start, share across turns, delete on task close.
  - *Maps to*: CAL-REQ-013.

- [ ] **CAL-TASK-0015**: Implement explicit lifecycle state machine (`OPEN -> CLOSING -> CLOSED`) and idempotent `RuntimeTask.close()` teardown.
...
- [ ] **CAL-TASK-0016**: Freeze semantic configuration and tool authority at task creation.
...
- [ ] **CAL-TASK-0017**: Implement per-turn `InvocationConfig` validation.
...
- [ ] **CAL-TASK-0018**: Enforce per-turn `invocation_id` routing, `ToolRequest` correlation, and `ToolExecutor.end_invocation(invocation_id)` cleanup.
...
- [ ] **CAL-TASK-0019**: Bind dynamic tools into Codex SDK mux for task turns and route requests safely.
...
- [ ] **CAL-TASK-0020**: Implement `RuntimeNode` task routing and mutual exclusion.
...
- [ ] **CAL-TASK-0021**: Propagate `task_id` to OpenTelemetry spans, LangSmith metadata, and turn events.
...
- [ ] **CAL-TASK-0022**: Migrate Smart Quote Agent example to `profile="controlled_turn"`.
  - Update `examples/smart_quote_agent/app.py` (Line 159), `graph.py`, `README.md`, and `tests/test_agent.py` to use `controlled_turn`.
  - *Maps to*: CAL-REQ-022.

- [ ] **CAL-TASK-0023**: Unit tests for profile resolution, capabilities, and factory boundaries.
...
- [ ] **CAL-TASK-0024**: Unit tests for task lifecycle, tool binding validation, instructions, concurrency, user-only replay protection, non-resumability, tool mux execution, and idempotent close.
...
- [ ] **CAL-TASK-0025**: Live Codex multi-turn smoke test with subscription authentication.
...
- [ ] **CAL-TASK-0026**: Validate and synchronize ADR 0004.
...
- [ ] **CAL-TASK-0027**: Synchronize Design Guide.
