...
- `AC-SQA-021`: **Interaction, Model, Tool, and HITL Latency Breakdown**
  **State:** `done`
  **Requirements:** `SQA-REQ-021`
  **Tasks:** `SQA-TASK-0019`
  *Criterion:* Metadata-only lifecycle events correlate interaction and model-call IDs. Inspector output separates host preparation, direct `RuntimeModel` call duration or `RuntimeTask` invocation span with tool cycles marked, TTFT or structured terminal latency, tool pre/post intervals, actual tool duration, approval wait, and discount wait. A per-invocation view includes only its correlated model call; interaction-level discount waits and invocation-level approval waits are labeled at their own scopes. Usage reports the final snapshot only and calculates cache ratio correctly. `--latency --limit N` groups the latest invocations by stage/model/profile and reports count, nearest-rank p50/p95, and weighted cache ratio; `--last` makes HITL-inclusive wall time clear. No new telemetry schema object or public runtime API is introduced.
  *Evidence:* The full example suite passes (159 passed, 2 skipped); strict mypy, Ruff lint/format, and `git diff --check` pass. Controlled inspector tests verify setup, TTFT/terminal, multi-phase tool timing, HITL, incomplete calls, final usage snapshots, nearest-rank percentiles, sibling-call isolation, correlation, and redaction. Live `--task`/`--interaction` reports show model spans, 13-ms tool execution, 3.54-second approval wait, and 3.90-second discount wait. See `05_validation_plan.md`.

- `AC-SQA-022`: **Byte-Stable and Bounded Cache-Eligible Prompts**
  **State:** `done`
  **Requirements:** `SQA-REQ-022`
  **Tasks:** `SQA-TASK-0020`
  *Criterion:* Structured decisions use byte-identical system prompts and schema serialization across identity, language, workflow, and quote contexts. Variable context appears only in compact UTF-8 JSON serialized with recursive lexicographic key sorting, `ensure_ascii=False`, separators `(',', ':')`, preserved array order, and a 16,384-byte cap; overflow causes a localized host clarification with no silent truncation or provider call. Controlled-agent instructions keep their stable prefix and role suffix at task creation without host-history replay. A provider cache hit is observed when available but is not required for acceptance.
  *Evidence:* Tests compare actual structured system prompt and schema bytes across initial and pending workflow contexts, verify sorted compact JSON, escaping, UTF-8 byte overflow rejection without model call, and preserved workflow. Live usage showed 94% cached input for the controlled-agent task; structured-router calls reported 0% cached tokens. A live cache hit is not mandatory. See `05_validation_plan.md`.

- `AC-SQA-023`: **One Structured Inference for Quote Classification and Extraction**
  **State:** `done`
  **Requirements:** `SQA-REQ-003`, `SQA-REQ-015`, `SQA-REQ-023`
  **Tasks:** `SQA-TASK-0021`
  *Criterion:* `TurnDecision` carries intent/language and at most one of `quote_request` or `quote_patch`; non-quote intents carry neither, while quote creation may carry either payload or neither. Initial/reformulated requests use `QuoteRequest`; a full reformulation replaces a pending workflow with a new `workflow_id` and invalidates its draft; pending-workflow edits use `QuotePatch`; missing information is preserved for host clarification. `quote_planner` performs host reduction only and issues no structured model call. The ambiguous baseline phrase still reaches Codex exactly once for its decision and yields Notebook Pro 1x and Wireless Mouse 2x. No keyword/regex fast paths are added, and host resolution/pricing/authorization/persistence remain authoritative.
  *Evidence:* Tests enforce payload exclusivity, one structured inference, no planner model call, and fresh workflow IDs for complete reformulations. The repeated live ambiguous phrase created one router invocation and reached Initech review with NB-PRO 1x, MS-WL 2x, USD 1,280 subtotal/total, and zero discount. Approval was denied; the quote count remained 9. See `05_validation_plan.md`.
